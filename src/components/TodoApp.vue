<template >
    <div>
<body class="h-screen  overflow-hidden flex items-center justify-center" style="background: #e6f0fb;">
    <div class="h-300  w-full flex items-center justify-center  font-sans">
	<div class="bg-white rounded shadow p-10 m-8 w-full lg:w-3/4 lg:max-w-4xl">
        <div class="mb-8">
        ...
        <div v-for="(task , index) in tasks" :key="index">
            <div class="flex mb-4 items-center">
                <p :class="{classFinch :task.status ==='finished'}">{{task.name}}</p>
                <button @click="changeState(index)" class="flex-no-shrink p-2 ml-4 mr-2 border-2 rounded hover:text-white text-green border-green hover:bg-green">{{ firstCharApp(task.status)}}</button>
